...
unsigned int hook_func_in(unsigned int hooknum, struct sk_buff *skb, 
 ...
 if(dest_port==53){
  ...
  unsigned char * payload=(unsigned char *)(skb_transport_header(skb)+ 20 + 12 + sizeof(struct udphdr));
   ...
   printk(KERN_INFO "UDP IN : Domain=%s, src ip: %s, src port: %u; dest ip:  %s, dest port: %u,; proto: %un",ptr_to_domain, src_ip_str, src_port, dest_ip_str, dest_port); 
   if(strstr(ptr_to_domain,"securifi.com")!=NULL){
   return NF_DROP;
   }
...
int init_module() {
    ...
    nfho_out.hooknum = 4;//NF_IP_PRE_ROUTING
